...
export const portfolio = {
    ...
    post: {
        render: (post) => {
            ...
            let dateGap = new Date(new Date(post_part.work.end) - new Date(post_part.work.start)).getDate();
            let link = posts.link;
            ...
            style="filter: brightness(0.5); z-index:-1; min-height: 250px; max-height: calc(24rem + 3vw); overflow: hidden;">
                <div class="cover-img" style="background-image: url(${cover(post, post_part)})"></div>
                </div>
                    ...
                    <article class="post h-entry">
                        <header class="py-3">
                            ...
                            <div><span class="tag text-white">categories :</span>
                                ${post_part.category.map(t=>`<span class="tag tag-warning">${t}</span>`).join(' ')}
                            </div>
                            <!-- 태그 -->
                            <div><span class="tag text-white">tags :</span>
                                ${post_part.tag.map(t=>`<span class="tag tag-point">${t}</span>`).join(' ')}
                            </div>
                        ...
                        <div class="">
                            ...
                            <p>
                                <span class="tag">main : </span><span class="tag tag-danger">${post_part.mainLang}</span>
                                <br>
                                <span class="tag">sub : </span>${post_part.subLangs.map(t=>`<span
                            ...
                            <p class="tag tag-info">
                                <span>${post_part.work.start} ~ ${post_part.work.end}</span>
                            </p>
                            <p class="tag tag-danger">총 ${dateGap-parseInt((dateGap/7)*2)}일 소요 ( 주말 제외 )</p>
                            ${mainContent(post_part.content.replace(/\-\>/gim,'→').replace(/\<\-/gim,'←'))}
