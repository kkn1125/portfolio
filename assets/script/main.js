  ...
  (function () {
    function Controller() {
      ...
      this.init = function (model) {
        ...
        window.addEventListener("keydown", this.handleSideMenu);
      ...
      this.aboveBtnScroll = function (ev) {
        ...
        if (target.classList.contains("btn")) {
          section.scrollTo({
            left: 0,
            top: section.scrollTop - ev.wheelDeltaY * 5,
            behavior: "smooth",
          });
        }
      };
      ...
      this.handleRollingList = function (ev) {
        ...
        target.textContent = parent.classList.contains("show")
          ? "📘less"
          : "📖more";
      };
      ...
      this.handleSideMenu = function (ev) {
        ...
        if (ev.type === "keydown") {
          if (ev.key === "Escape") {
            models.handleSideMenu(null, sibling);
          } else {
            return;
          }
        } else {
      ...
      this.handleLink = function (ev) {
        ...
        if (
          ...
          anchor.getAttribute("href").indexOf("http") > -1
        ) {
          ev.preventDefault();
      ...
      this.blockRoutAnchor = function (ev) {
        ...
        if (
          anchor.tagName !== "A" ||
          !anchor.getAttribute("href").match(/\#/gm)
        )
          return;
        ev.preventDefault();
    ...
    function Model() {
      ...
      const state = {
        animations: ["fade", "zoom", "start", "end", "top", "bottom"],
        view: "animation",
      };
      ...
      this.handlePaging = function (target) {
        ...
        const alerting = (page = false) => {
          ...
          let notice = target.parentNode.insertAdjacentElement(
            "afterend",
        ...
        this.routeAnchor(
          `#${pages[pageIdx]}${
            subpages[subpageIdx] ? "-" + subpages[subpageIdx] : ""
          }`
        );
      };
      ...
      this.getRandomMotion = function () {
        motion =
          state.animations[parseInt(Math.random() * state.animations.length)];
      };
    }
    ...
    function View() {
      ...
      this.renderContext = function (x, y) {
        ...
        document.body.insertAdjacentHTML(
          "beforeend",
          moduler.contextmenu.render(moduler.contextmenu)
        );
        if (
          window.innerHeight * 1.2 <
          y + document.querySelector(".contextmenu").clientHeight
        ) {
        ...
        if (
          ...
          x + document.querySelector(".contextmenu").clientWidth
        ) {
      ...
      this.renderingInitialItems = function () {
        for (let type in parts) {
          app.insertAdjacentHTML(
            "beforeend",
            moduler.pages.item(type).render(location.hash)
          );
        }
      };
        ...
        try {
          ...
          if (page == "portfolio" && !subpage) {
            ...
            const prjs = Object.entries(moduler.projects).sort((a, b) => {
              return (
                new Date(b[1].writedAt).getTime() -
                new Date(a[1].writedAt).getTime()
              );
            });

            for (let [prj, sources] of prjs) {
              document
                .querySelector(`[page="${page}"] .card-group`)
                .insertAdjacentHTML(
                  "beforeend",
                  moduler.router[page]
                    .select(page)
                    [mode].render(moduler.projects[prj], prj)
                );
            }
            ...
            document
              ...
              .insertAdjacentHTML(
                "beforeend",
                moduler.router[page].select(page)["post"].render(subpage)
              );
          }
        } catch (e) {
          app.insertAdjacentHTML(
      ...
      this.clearPage = function () {
        ...
        const oldSide = app.querySelectorAll(
          `[class*="side-"], [id="side-menu"]`
        );
        ...
        oldPage.forEach((old) => {
          ...
          if (!oldClass.match(/[\w]+-[\w]+/gm)) {
            if (childrenCount > 1)
              old.classList.replace(oldClass, `out-${oldClass}`);
                            ...
                            <div class="contextmenu">
                                ...
                                <ul class="list-group py-0">
                                    ...
                                    ${menu
                                      .map(
                                        (li) =>
                                          `<li class="list-item py-1"><a href="#${li}">${li}</a></li>`
                                      )
                                      .join("")}
                                </ul>
                                <div class="hr"></div>
